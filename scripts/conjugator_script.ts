...
// extend the HTML input box so that it has the methods to convert to/from
interface VerbInput extends HTMLInputElement {
  toRoot?: (verb: string) => string;
  fromRootHeavy?: (verb: string) => string;
  fromRootLight?: (verb: string) => string;
  changed?: (verb: string) => void;
}

// helper function to get hold of the box element needed for input/output
function getBox(id: string): VerbInput {
  return document.getElementById(id) as VerbInput;
}


// to avoid faffing with dotted letters, safe to assume anything with an e is heavy?
// heavy vowels = true, light (Dotted) = false
function getVowelGroup(verb: string): Boolean {
  if (verb.includes('e')) { 
    var vowelgroup = true;
  } else {
...
const vowelgroupbox = document.getElementById('vowelgroupbox') as HTMLInputElement
...
function update_form(event: Event): void {
  ...
  const changed_box = event.target as VerbInput;
  if (!changed_box.toRoot) return;
// calculate the energy in Joules from the changed box
  ...
  for (const box of verbboxes) {
    if (box === changed_box || !box.fromRootHeavy || !box.fromRootLight ) continue;
    if (vowelgroup) {
