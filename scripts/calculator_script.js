...
energykcalmolbox.toJoule = () => (1000 * kcal * Number(energykcalmolbox.value)) / avogadro;
...
angwavenumberbox.toJoule = () => planck * c * (Number(angwavenumberbox.value) / (2 * pi));
...
boltzmannbox.toJoule = () => Math.log(Number(boltzmannbox.value) / Number(boltzmanngbox.value)) * boltz * Number(boltzmannTbox.value);
boltzmannbox.fromJoule = (energyJ) => Number(boltzmanngbox.value) *
    Math.exp(-energyJ / (boltz * Number(boltzmannTbox.value)));
// gives K = exp(-dG/kT)
const equilibriumconstantbox = getBox("equilibrium");
equilibriumconstantbox.toJoule = () => -boltz *
    Number(boltzmannTbox.value) *
    Math.log(Number(equilibriumconstantbox.value));
equilibriumconstantbox.fromJoule = (energyJ) => Math.exp(-energyJ / (boltz * Number(boltzmannTbox.value)));
// gives lnK = -dG/kT
const lnequilibriumconstantbox = getBox("lnequilibrium");
lnequilibriumconstantbox.toJoule = () => -boltz *
    Number(boltzmannTbox.value) *
    Number(lnequilibriumconstantbox.value);
lnequilibriumconstantbox.fromJoule = (energyJ) => -energyJ / (boltz * Number(boltzmannTbox.value));
// Arrhenius A factor
...
arrheniusbox.toJoule = () => -Math.log(Number(arrheniusbox.value) / Number(Aconstantbox.value)) *
    boltz *
    Number(boltzmannTbox.value);
arrheniusbox.fromJoule = (energyJ) => Number(Aconstantbox.value) *
    Math.exp(-energyJ / (boltz * Number(boltzmannTbox.value)));
// half life from Arrhenius assuming 1st order kinetics t = ln2 / k 
const halflifebox = getBox("halflife");
halflifebox.toJoule = () => -Math.log((Math.log(2) / Number(halflifebox.value)) / Number(Aconstantbox.value)) *
    boltz *
    Number(boltzmannTbox.value);
halflifebox.fromJoule = (energyJ) => Math.log(2) /
    (Number(Aconstantbox.value) *
        Math.exp(-energyJ / (boltz * Number(boltzmannTbox.value))));
// cell potential, E = -dG/nF
const cellpotentialbox = getBox("cellpotential");
cellpotentialbox.toJoule = () => (-Number(nelectronsbox.value) * faraday * Number(cellpotentialbox.value)) /
    avogadro;
cellpotentialbox.fromJoule = (energyJ) => (energyJ * avogadro) / (-Number(nelectronsbox.value) * faraday);
...
boltzmannTbox.addEventListener("input", () => { var _a; return (_a = boltzmannTbox.changed) === null || _a === void 0 ? void 0 : _a.call(boltzmannTbox, Number(energyJbox.value)); });
boltzmanngbox.addEventListener("input", () => { var _a; return (_a = boltzmanngbox.changed) === null || _a === void 0 ? void 0 : _a.call(boltzmanngbox, Number(energyJbox.value)); });
nelectronsbox.addEventListener("input", () => { var _a; return (_a = nelectronsbox.changed) === null || _a === void 0 ? void 0 : _a.call(nelectronsbox, Number(energyJbox.value)); });
Aconstantbox.addEventListener("input", () => { var _a; return (_a = Aconstantbox.changed) === null || _a === void 0 ? void 0 : _a.call(Aconstantbox, Number(energyJbox.value)); });
