...
const normalisecb = document.getElementById("normalisecb");
const autoscalecb = document.getElementById("autoscalecb");
...
function sliderToD(fraction, B) {
    return B * d_ratio_max * fraction ** d_slider_power;
}
function dToSlider(D, B) {
...
function addLorentzian(spec, x, step, w0, gamma, A) {
    const n = x.length;
...
function spectrumFromLines(x, step, lines, linewidth) {
    const spec = new Array(x.length).fill(0);
...
function fitRange(lines, linewidth, top) {
...
// ── current state ─────────────────────────────────────────────────────────────
function currentMaxJ() {
    let m = Math.floor(Number(jmaxInput.value));
    if (!isFinite(m) || m < 1)
        m = 1;
    if (m > maxJ_cap)
        m = maxJ_cap;
    if (String(m) !== jmaxInput.value)
        jmaxInput.value = String(m);
    return m;
}
