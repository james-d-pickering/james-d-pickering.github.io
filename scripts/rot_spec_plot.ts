...
const kValue = document.getElementById("kValue") as HTMLSpanElement;
...
let rs: number[] = linspace(1E-6, 12, 1000);

function DLVO_attraction(r: number[], a: number, deg: number = Number(attdropdown.value)) {
    const ys: number[] = [];
...
function hard_sphere_repulsion(r: number[]) {
    const ys: number[] = [];
...
function DLVO_repulsion(r: number[],b: number, k: number, potential: string = String(repdropdown.value)) {
    const ys: number[] = [];
...
function combined_DLVO(hs: number[], att: number[], rep: number[]) {
    const combined: number[] = att.map((val, idx) => val + rep[idx] + hs[idx]);
    return combined;
}
    ...
    options: {
        ...
        scales: {
            ...
            y: {
                ...
                min: -0.2,
                max: 0.3,
                afterBuildTicks: (axis: { ticks: { value: number; }[]; }) => axis.ticks = [0].map(v => ({ value: v })) 
            }
        },
        elements: {
            point: {
